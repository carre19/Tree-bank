...
CREATE TABLE IF NOT EXISTS Personas (
    ...
    password_hash VARCHAR(255),  -- para el login del homebanking
...
CREATE TABLE IF NOT EXISTS Movimientos (
    ...
    tipo_movimiento VARCHAR(50) NOT NULL, -- 'TRANSFERENCIA_EGRESO', 'TRANSFERENCIA_INGRESO'
    monto DECIMAL(15, 2) NOT NULL,
    descripcion VARCHAR(255),
    referencia_externa VARCHAR(100) UNIQUE, -- ID de transacción del Banco Central (para evitar duplicados en sync)
    fecha TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (id_cuenta) REFERENCES Cuentas_Bancarias(id_cuenta)
);

-- 9. POBLAR TABLAS MAESTRAS (Datos iniciales)
